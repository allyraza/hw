...
int main(int argc, char** argv) {
		 ...
		 check_consistency(1);
		 printf("passed first consistency check\n");
		 for(int i=0;i<MAX_ALLOCATIONS;i++) 
			 myfree(allocs[i]);
		 printf("Passed first round of allocations / frees\n");
		 ...
		 printf("Simple alloc/free, alloc/free cycle: %ld, timediff:%d\n",sbrk(0)-pre,timediff());
		 ...
		 printf("Allocated lots and freed all but one: %ld, timediff:%d\n",sbrk(0)-pre,timediff());
		 ...
		 printf("Coalescing free and splitting mymalloc: %ld, timediff:%d\n",sbrk(0)-pre,timediff());
		 ...
		 printf("After freeing coalesced blocks: %ld, timediff:%d\n",sbrk(0)-pre,timediff());
		 ...
		 printf("Random malloc and free sequence: %ld, timediff: %d\n",sbrk(0)-pre,timediff());
		 pre=sbrk(0);
